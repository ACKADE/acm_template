...
> **解决问题**：在单峰/单谷函数上快速找到极值点
> **时间复杂度**：O(log n)
> **适用场景**：求凸函数/凹函数的极值点，不要求可导

---

### 倍增

> **解决问题**：通过预处理 2^k 级祖先/跳跃信息，将 O(n) 的步进操作加速到 O(log n)
> **时间复杂度**：预处理 O(n log n)，查询 O(log n)
> **适用场景**：ST 表、LCA 倍增、树上 k 级祖先、快速幂
...
> **解决问题**：线性时间维护滑动窗口内的最值或维护单调递增/递减序列
> **时间复杂度**：O(n)
> **适用场景**：滑动窗口最值、最近更大/更小元素、最大矩形面积
...
> **解决问题**：动态维护集合中的最值，支持插入和删除最值
> **时间复杂度**：插入/删除 O(log n)，查询最值 O(1)
> **适用场景**：优先队列（STL priority_queue 已实现）、Dijkstra

#### 左偏树 / 可并堆【待补充】

> **解决问题**：支持快速合并两个堆的数据结构
> **时间复杂度**：合并 O(log n)
> **适用场景**：需要合并两个堆时（如某些树上合并问题）
...
> **解决问题**：在程序中表示图的结构
> **时间复杂度**：邻接矩阵 O(V^2)，邻接表/链式前向星 O(V+E)
> **适用场景**：根据图的稠密程度和算法需求选择合适的存储方式
