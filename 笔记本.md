...
**游戏规则**：$n$ 级台阶编号 $1 \sim n$（$1$ 为地面），第 $i$ 级有 $a_i$ 颗石子。每步从第 $i$ 级（$i>1$）取任意正数颗石子**下移**到第 $i-1$ 级。
...
**变式与注意**：若地面编号为 $0$ 且石子移到地面后移除，则等价于对**奇数索引**做 Nim。经典模型：POJ 1704（Georgia and Bob，棋子左移=石子下移）。
...
**推导思路**：
1. 用 Fusion 原理消去所有环 → 化为树。
2. 用 Colon 原理从叶子向根递推 SG。
3. 总 SG = 各连通分量 SG 的异或。
...
**游戏规则**：一排 $n$ 枚硬币，每枚正面 (H) 或反面 (T)。每步选择一个"允许的翻转模式"，将对应的硬币翻转（H↔T）。最右侧被翻转的硬币必须**从 H 翻为 T**。

**结论（Turning Turtles 定理）**：若满足上述规则，则局面的 SG 值等于**各 H 硬币所在位置 SG 值的异或**。其中每个位置 $i$ 的 SG 值等于该位置作为一维减法游戏时的 SG 值。

**核心模型——Turning Turtles**：翻转位置 $i$（必须 H→T），并可选择翻转 $i$ 左侧任意一枚硬币。
...
**游戏规则**：游戏由多个独立子游戏组成。每步玩家必须在**每一个尚未结束的子游戏**中各走一步。任一子游戏中无法走即判负。
...
> **解决问题**：求解形如 $x_i - x_j ≤ c$ 的不等式组，通过建图跑最短路来实现
> **时间复杂度**：O(n*m)
> **适用场景**：区间限制转化为图论约束
...
> **适用场景**：多次询问涉及少量关键点（≤10^5 总关键点数）的树上问题
