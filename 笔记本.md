...
for (int len = 2; len <= n; len++) {
        for (int l = 1; l + len - 1 <= n; l++) {
            ...
            for (int k = opt[l][r - 1]; k <= opt[l + 1][r]; ++k) {
                //枚举l到r中间的分割点
                int val = dp[l][k] + dp[k + 1][r] + w(l, r);
                if (val < dp[l][r]) {
                    dp[l][r] = val, opt[l][r] = k;
                }
            }
        }
    }
...
**分治法**：  
特别适合**带层数**的 DP,对于当前层，递归计算 $dp[l..r]$，已知其决策点在 $[optL, optR]$ 内，取中点枚举决策，均摊 $O(n\log n)$ 每层。  

```c++
// 计算第 k 层 dp[i][k] (i ∈ [l, r])
// 决策点 j 必须 < i 且 j ∈ [optL, optR]
void compute(int l, int r, int optL, int optR) {
    ...
    dp[mid][k] = minv; // 填当前层的 dp 值
    compute(l, mid - 1, optL, best); // 左半
    compute(mid + 1, r, best, optR); // 右半
}
//下面为主函数调用
for (int k = 2; k <= K; k++) {
	for (int i = 0; i <= n; i++) {
        pre[i] = dp[i][k - 1]; // 上一层结果
    }
    compute(k, n, k - 1, n); // i 从 k 开始（至少 k 个元素），j 至少是 k-1
}
```
...
deque<Decision> dq;
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        while (!dq.empty()) {
            auto &tail = dq.back();
            int j = tail.j;
            if (dp[i] + w(i, tail.l) <= dp[j] + w(j, tail.l)) {
                dq.pop_back();
            } else {
                break;
            }
        ...
        if (dq.empty()) {
            ...
            auto &tail = dq.back();
            int j = tail.j;
            int left = tail.l, right = tail.r, pos = tail.r + 1;
            while (left <= right) {
                int mid = (left + right) >> 1;
                if (dp[i] + w(i, mid) <= dp[j] + w(j, mid)) {
                    pos = mid;
                    right = mid - 1;
                } else {
                    left = mid + 1;
...
> **时间复杂度**：$O(n \log n)$（树状数组优化）或 $O(n^2)$（朴素）
...
> **解决问题**：并查集维护每个点到根节点的「距离」或「关系」（加法/异或/模意义下），路径压缩时同步更新权值
> **时间复杂度**：O(α(n))
> **适用场景**：食物链类关系判定、奇偶游戏、模意义下的等价关系维护
