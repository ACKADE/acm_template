...
class MinCostFlow {
public:
    MinCostFlow(int n) : n(n), g(n), h(n), dist(n), vis(n) {}

    int add_edge(int u, int v, int cap, int cost) {
        int x = g[u].size();
        int y = g[v].size();
        if (u == v) ++y;  // 自环修正
        g[u].push_back({v, y, cap, cap, cost});
        g[v].push_back({u, x, 0, 0, -cost});
        return x;
    }

    pair<int, int> min_cost_max_flow(int s, int t) {
        return min_cost_flow(s, t, numeric_limits<int>::max() / 4);
    }
...
private:
    ...
    static constexpr int INF = numeric_limits<int>::max() / 4; // 关键修正

    int n;
    vector<vector<Edge>> g;
    vector<int> h, dist;
    vector<bool> vis;  // 回溯式防环标记
