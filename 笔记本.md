...
> **解决问题**：提供比赛代码的统一入口、常用宏和多组数据框架
> **时间复杂度**：框架本身 $O(1)$，总复杂度取决于 `solve()`
> **适用场景**：新建题目代码时直接复制，按题目修改常量与多测读入
...
#define endl '\n'
using namespace std;
const int N = 200005;          // 按题目数据范围修改
const int MOD = 998244353;     // 按题目模数修改
...
void build_hash(const string &s) {
    ...
    for (int i = 1; i <= n; ++i) {
        p[i] = p[i - 1] * base % mod;
        hash[i] = (hash[i - 1] * base + (unsigned char)s[i - 1]) % mod;
    }
}
...
```c++
const int MOD = 998244353;  // 必须为质数，以下运算在有限域 F_MOD 上进行
...
struct gaosi {
    ...
    // 返回值：0=无解，1=唯一解，2=多解（共有 MOD^(m-rk) 个解）
    int solve(vector<vector<int>> a, int n, int m) {
        // 全局统一取模，保证所有元素落在 [0, MOD)
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j <= m; ++j) {
        ...
        if (rk < m) return 2;  // 多解，存在 m-rk 个自由变量
        // 提取唯一解
        for (int i = 0; i < m; ++i)
            ans[i] = a[i][m];
        return 1;
...
struct gaosi_xor {
    ...
    // n个方程，m个未知数
    // 返回值：0=无解，1=唯一解，2=多解（共有 2^(m-rk) 个解）
...
// res=0 无解；res=1 唯一解，答案存在 gx.ans[0~m-1]；res=2 多解
...
#### 无源汇上下界最小费用可行循环流【推导草稿：未做负环消去，勿直接提交】
...
#### 有源汇上下界最小费用可行流【推导草稿：未完成独立验证，勿直接提交】
...
#### 有源汇上下界最小费用最大流【推导草稿：未完成独立验证，勿直接提交】
...
#### 有源汇上下界最小费用最小流【推导草稿：未完成独立验证，勿直接提交】
...
public:
    ...
    pair<int, int> solve(int s, int t) {
        ...
        // 只抵消已有的s->t流，最小流量按非负值定义
        auto [dec_flow, dec_cost] = mcmf(t, s, init_flow);
