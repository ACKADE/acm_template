...
const int MAXN = 305;       // 最大未知数/方程数，赛场按需调整
struct gaosi {
    int ans[MAXN];  // 存储唯一解的答案
    gaosi() {
    ...
    // 传入增广矩阵a，n个方程，m个未知数
    // 增广矩阵：n行m+1列，a[i][0~m-1]是系数，a[i][m]是常数项
    // 返回值：0=无解，1=唯一解，2=无穷多解
    int solve(vector<vector<int>> a, int n, int m) {
        // ========== 关键修正：全局统一取模，保证所有元素落在 [0, MOD) ==========
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j <= m; ++j) {
...
// 未知数编号：0 ~ m-1
// 方程编号：0 ~ n-1
// 增广矩阵列号：0 ~ m-1 为系数列，第 m 列为方程右侧常数项

// 1. 实例化高斯消元计算器
gaosi gs;

// 2. 构造增广矩阵
// n 个方程，m 个未知数，矩阵大小为 n 行 × (m+1) 列
int n = 3, m = 3;
...
// 支持直接传入负数、大于 MOD 的数值，函数内部已内置全局标准化取模

// 3. 求解方程组
int res = gs.solve(a, n, m);

// 4. 根据返回值判断解的情况
// res == 0：方程组无解
// res == 1：有唯一解，答案存储在 gs.ans[0 ~ m-1] 中，ans[i] 对应第 i 个未知数的解
// res == 2：有无穷多组解
...
```c++
const int MAXM = 1005;      // 最大未知数个数，赛场按需调整
struct gaosi_xor {
    bitset<MAXM + 1> a[MAXM];  // 增广矩阵，a[i][0~m-1]为系数，a[i][m]为常数项
    int ans[MAXM];             // 存储唯一解的答案
...
// 填入增广矩阵：第 i 个方程
// gx.a[i][j] = 第 j 个未知数的系数（0 或 1）
// gx.a[i][m] = 方程右侧常数项（0 或 1）

// 求解，n 个方程，m 个未知数
int res = gx.solve(n, m);
// res=0 无解；res=1 唯一解，答案存在 gx.ans[0~m-1]；res=2 无穷多解
...
public:
    ...
    pair<double, vector<pair<int, int> > > solve() {
        ...
        for (auto &[u, v, w]: edges) {
            ...
            int sz = n;
            vector<double> prefA(sz), sufB(sz);
            for (int i = 0; i < sz; ++i) {
                int node = pts[i].second;
                prefA[i] = d[u][node];
                if (i > 0) prefA[i] = max(prefA[i], prefA[i - 1]);
            }
            for (int i = sz - 1; i >= 0; --i) {
                int node = pts[i].second;
                sufB[i] = d[v][node];
                if (i + 1 < sz) sufB[i] = max(sufB[i], sufB[i + 1]);
            }
            ...
            while (start < sz) {
                ...
                double Lmax = prefA[end]; // 左侧 max d[u][k]
                double Rmax = (end + 1 < sz ? sufB[end + 1] : -INF); // 右侧 max d[v][k]
                ...
                double ecc = max(Lmax + x0, Rmax + w - x0);
...
mcf.add_edge(u, v, 容量, 单位费用);    // 自动建反向残量边，返回正向边在 u 出边中的下标
// 双向边：正反各调用一次 add_edge；重边直接多次调用，无需合并，算法自动择优增广

// 2. 跑费用流
// 每次调用跑流函数都会自动重置所有边为初始容量，支持多次独立调用
// ① 最小费用最大流（最常用），返回 pair<实际最大流量, 对应最小总费用>
auto [max_flow, min_cost] = mcf.min_cost_max_flow(源点, 汇点);

// ② 指定流量最小费用，求恰好流 need 单位的最小费用
// 若返回流量 < need，说明图中无法达到指定流量
auto [flow, cost] = mcf.min_cost_flow(源点, 汇点, need);

// 3. 获取单条边实际流量（跑流完成后使用）
// idx 按 add_edge 为 u 添加边的顺序从 0 开始，正向边、反向边均计入索引
// 正向边流量为正，反向边为负（表示退流）
// 推荐用 add_edge 返回值记录正向边序号，避免手动计数出错
int f = mcf.get_flow(u, 边序号);
...
public:
    ...
    void add_edge(int u, int v, int low, int high) {
...
解决问题：满足所有边流量上下界约束的前提下，求源点到汇点的最小流量 时间复杂度：$O(m \cdot n^2)$ 适用场景：带下界约束的最小流计算、最少流量方案判定
...
解决问题：判断无源汇网络中是否存在满足上下界的循环流，若存在则返回最小总费用 时间复杂度：$O(F \cdot E \log V)$，$F$ 为总强制流量 适用场景：带费用的循环流优化、无源头汇点的流量费用最小化
...
public:
    ...
    BoundCostCircuitFeasible(int n) : n(n), S(n), T(n + 1), g(n + 2), in(n + 2, 0),
                                      h(n + 2), dist(n + 2), cur(n + 2), vis(n + 2),
                                      timer(0), base_cost(0) {}

    // 加一条 u->v 的有向边，流量下界 low，上界 high，单位费用 cost，返回正向边下标
    ...
    pair<bool, int> solve() {
        ...
        auto [flow, cost] = mcmf(S, T, total);
        if (flow != total) return {false, 0};
...
private:
    ...
    int dfs(int u, int t, int f) {
        ...
        for (int &i = cur[u]; i < (int)g[u].size(); ++i) {
            ...
            if (e.cap == 0 || vis[e.to] == timer) continue;
            if (e.cost + h[u] - h[e.to] != 0) continue;
            int pushed = dfs(e.to, t, min(f - total, e.cap));
            if (pushed) {
                e.cap -= pushed;
                g[e.to][e.rev].cap += pushed;
                total += pushed;
    ...
    pair<int, int> mcmf(int s, int t, int need) {
        spfa_init(s);
        int flow = 0, cost = 0;
        while (flow < need && dijkstra(s, t)) {
            for (int v = 0; v < n + 2; ++v)
...
```c++
// 1. 建图
BoundCostCircuitFeasible bmf(n);
bmf.add_edge(u, v, low, high, cost);

// 2. 求解
auto [ok, min_cost] = bmf.solve();
```
...
解决问题：判断有源汇网络中是否存在满足上下界的可行流，若存在则返回最小总费用 时间复杂度：$O(F \cdot E \log V)$ 适用场景：带下界与费用的流存在性判定、最小费用可行方案求解
...
```c++
// 1. 建图
BoundCostFlowFeasible bmf(n);
bmf.add_edge(u, v, low, high, cost);

// 2. 求解 s 到 t 的最小费用可行流
auto [ok, min_cost] = bmf.solve(s, t);
```
...
解决问题：满足上下界约束且流量最大的前提下，求最小总费用 时间复杂度：$O(F \cdot E \log V)$ 适用场景：竞赛高频考点，带下界的最小费用最大流问题
...
public:
    ...
    pair<int, int> solve(int s, int t) {
        ...
        auto [flow1, cost1] = mcmf(S, T, total);
        if (flow1 != total) return {-1, -1};

        int init_flow = g[t][ts_idx].init_cap - g[t][ts_idx].cap;
        int total_cost = base_cost + cost1;
        ...
        auto [add_flow, add_cost] = mcmf(s, t, INF);
        return {init_flow + add_flow, total_cost + add_cost};
    }
...
```c++
// 1. 建图
BoundCostMaxFlow bmf(n);
bmf.add_edge(u, v, low, high, cost);

// 2. 求解 s 到 t 的最小费用最大流
auto [max_flow, min_cost] = bmf.solve(s, t);
if (max_flow == -1) cout << "No Solution\n";
else cout << max_flow << ' ' << min_cost << '\n';
```
...
解决问题：满足上下界约束且流量最小的前提下，求最小总费用 时间复杂度：$O(F \cdot E \log V)$ 适用场景：带下界的最小费用最小流问题、最少流量下的费用优化
...
public:
    ...
    pair<int, int> solve(int s, int t) {
        ...
        auto [dec_flow, dec_cost] = mcmf(t, s, INF);
        return {init_flow - dec_flow, total_cost + dec_cost};
    }
...
```c++
// 1. 建图
BoundCostMinFlow bmf(n);
bmf.add_edge(u, v, low, high, cost);

// 2. 求解 s 到 t 的最小费用最小流
auto [min_flow, min_cost] = bmf.solve(s, t);
if (min_flow == -1) cout << "No Solution\n";
else cout << min_flow << ' ' << min_cost << '\n';
```
...
3. `get_flow` 返回的是该边的**实际总流量**（已包含下界流量），下标规则与原有板子完全一致。
...
// 寻找当前子树的重心
void get_rt(int dq, int fu, int tot, int &rt) {
    int maxp = 0; // 最大子树大小
    for (auto to : ljb[dq]) {
        if (to == fu || vis[to]) continue;
        get_rt(to, dq, tot, rt);
    ...
    if (maxp < sz[rt]) rt = dq; // sz[0] 初始设为极大值
}
...
void solve(int dq) {
    get_sz(dq, 0);
    int rt = 0;
    sz[0] = 1e9; // 初始设为极大值，保证第一次会更新重心
    get_rt(dq, 0, sz[dq], rt);
    vis[rt] = true;
